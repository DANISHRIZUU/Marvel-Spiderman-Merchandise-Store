...
class Funko(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2)
    description = models.TextField()
    image = models.ImageField(upload_to='funko/', null=True, blank=True)
...
class Cart(models.Model):
    costume = models.ForeignKey(Costume, on_delete=models.SET_NULL, null=True)
    funko = models.ForeignKey(Funko, on_delete=models.SET_NULL, null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)
    quantity = models.IntegerField(default=0)

    def __str__(self):
        if self.costume:
            return f"{self.user.username} - {self.costume.name}"
        elif self.funko:
            return f"{self.user.username} - {self.funko.name}"
        return f"{self.user.username} - Empty Cart Item"
